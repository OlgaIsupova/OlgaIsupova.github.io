...
R. Balaniuk, **O. Isupova**, S. Reece. ``Mining and tailings dam detection in satellite imagery using deep learning'' *Sensors*, 2020  
...
**O. Isupova**, Y. Li, D. Kuzin, S. J. Roberts, K. Willis, S. Reece. ``BCCNet: Bayesian classifier combination neural network'' *NeurIPS Workshop on ML for Developing World*, 2018  
...
{% for year in site.data.papers %}
 ...
 {% for paper in year.papers %}
   * {{ paper.authors | array_to_sentence_string | markdownify}}. 
     ``{{ paper.title}}'' 
     *{{ paper.where}}*
     {% if paper.links %} \\
